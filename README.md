...
Simple backup script made in Python with option to send via sftp.

Collects files and folders and create a single zip file with given name and date.

#### Functionality

- Collect files and folders into single `ZIP` file.
...
```
When the `venv` is created, if you plan to send the backup to en `SFTP` server, the `paramiko` Python package is needed.
...
```bash
#!/root/.venv/oFileBackup/bin/python3
```
...
```bash
chmod 770 path/to/the/script/oFileBackup.py
```
